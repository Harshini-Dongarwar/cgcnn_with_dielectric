# <<< PASTE the full contents of your updated predict.py here >>>
import argparse
import os
import shutil
import sys
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        with torch.no_grad():
            if args.cuda:
                 input_var = (Variable(input[0].cuda(non_blocking=True)), #atom_fea
                         Variable(input[1].cuda(non_blocking=True)), #nbr_fea
                         input[3].cuda(non_blocking=True),#nbr_fea_idx
                         [crys_idx.cuda(non_blocking=True) for crys_idx in input[4]], #crystal_atom_idx
                         input[2].cuda(non_blocking=True) ) #Dielectric_fea
            else:
